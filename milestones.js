...
button.addEventListener("click", function () {
  ...
  const maxCarriage = 19;
  carriageResult = Math.floor(randomCarriage * maxCarriage) + 1;
  //GENERO IL NUMERO DI BIGLIETTO
  const randomTicket = Math.random();
  const maxTicket = 99998;
  ticketResult = Math.floor(randomTicket * maxTicket) + 1;
  ...
  if (discount) {
    ...
    discountPriceElement.innerHTML = `Il prezzo scontato è: € ${finalPrice.toFixed(
      2
    )}`;
  }
});
